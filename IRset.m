...
switch field
  ...
  case {'regbilStopTol'}% real non-negative scalar
    [validvalue, errmsg] = nonNegscalar(field,value);
  case {'truepinv'} %  off, on
    [validvalue, errmsg] = onOffType(field,value);
  case {'updateWeights'} % off, on
    [validvalue, errmsg] = onOffType(field,value);
  otherwise
    %validfield = false;  
    validvalue = false;
...
valid =  ischar(value) && any(strcmpi(value,{'none';'dwt';'dct';'svd';'tv1D';'tv2D';'atv2D';'diag'}));
if ~valid
  errmsg = sprintf('Invalid value for OPTIONS parameter %s: must be ''none'' or ''dwt'' or ''dct'' or ''svd'' or ''diag'' or ''tv1D'' or ''tv2D''.',field);
else
  errmsg = '';
end
...
valid =  (ischar(value) && any(strcmpi(value,{'none';'Identity';'Laplacian1D';'Laplacian2D';'Gradient1D';'Gradient2D';'tv';'tv1D';'tv2D';'diag';'atv2D'}))) || ...
...
valid =  (isreal(value) && isscalar(value) && (value >= 0)) | (ischar(value) && any(strcmpi(value,{'gcv','wgcv','modgcv','optimal','discrep','discrepit','Lcurve','off','none'})));
...
% One of these strings: tv, nn
valid =  (ischar(value) && any(strcmpi(value,{'I';'R';'Rnew'})));
if ~valid
  errmsg = sprintf('Invalid value for OPTIONS inSolver %s: must be ''I'' or ''R'' or ''Rnew''.',field);
else
  errmsg = '';
end
