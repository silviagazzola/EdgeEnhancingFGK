...
%                   'Gradient1D' | 'Gradient2D' | 'tv1D' | 'tv2D' | 'diag']
...
elseif ismatrix(L) && ~ischar(L)
    ...
    elseif ( strcmpi(L, 'Gradient1D') || strcmpi(L, 'Gradient2D') || strcmpi(L, 'tv1D') || strcmpi(L, 'tv2D') || strcmpi(L, 'diag') || strcmpi(L, 'atv2D') )
        ...
        elseif strcmpi(L,'diag')
            ...
            if ischar(weightx) % no weights given
                ...
                info.weight1 = ones(2*sqrtn*(sqrtn-1),1); %allocate prevweight value
                ...
                if max(abs(weightx)) == 0 % all weights zero
                    weightx = speye(2*sqrtn*(sqrtn-1));
                    info.weight1 = ones(2*sqrtn*(sqrtn1-1),1);
                    L = @(xx,tflag) SNrightPrec_tv2D(xx, weightx, u, v, Q, pinvDiag, sizeP, tflag);
                else
                    ...
                    info.weight1 = g; % the noninverse weights are passed through to outer next iteration
                    
                    g = 1-g; % alternative 'inverse' weights
                    g(g<tolX) = tolX; % ensure SVD is possible
                    % g = 1./g; % actual inverse weights
%                     % second approach
%                     g = ((abs(g)/max(abs(g))).^q).*prevweight ;% calculate weights
%                     g = g/abs(g); %%%
%                     info.weight1 = g; % the noninverse weights are passed through to next iteration
                    
                    weightx = spdiags(g ,0, 2*sqrtn*(sqrtn-1),2*sqrtn*(sqrtn-1)); %inverse weight matrix
                    
                    L = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u, v, Q, pinvDiag, sizeP, p1, p2, tflag);
                end
            end
        ...
        elseif strcmpi(L, 'tv1D')
            ...
            if ischar(weightx)
                ...
                if max(abs(weightx)) == 0
                    ...
                    %                     fprintf('Max weight: %g \t Min weight: %g.\n',max(abs(weightx)),min(abs(weightx)))
        ...
        elseif strcmpi(L, 'tv2D')
            ...
            if ischar(weightx) % no weights given
                ...
                if max(abs(weightx)) == 0 % all weights zero
                    ...
                    %                   fprintf('Max weight: %g \t Min weight: %g.\n',max(abs(weightx)),min(abs(weightx)))
                    weightx = spdiags(1./[weightx;weightx] ,0, 2*sqrtn*(sqrtn-1),2*sqrtn*(sqrtn-1));
                    L = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u, v, Q, pinvDiag, sizeP, p1, p2, tflag);
                end
            end % end of the 'if ischar(weightx)' block
            
            
            % PRECONDITIONING: Anisotropic total variation
        elseif strcmpi(L, 'atv2D')
            lname='atv2D';
            sqrtn = sqrt(n); % dimesion of the square 2D array
            ...
            if ischar(weightx) % no weights given
                weightx = speye(2*sqrtn*(sqrtn-1));
                    L = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
            else
                try
                    isequal(length(weightx),2*sqrtn*(sqrtn-1));
                catch
                ...
                if max(abs(weightx)) == 0 % all weights zero
                    ...
                    Dhx = weightx(:,1:sqrtn-1)-weightx(:,2:sqrtn);
                    Dvx = weightx(1:sqrtn-1,:)-weightx(2:sqrtn,:);
                    weightx =  abs([Dhx(:);Dvx(:)]+tolX).^((q-2)/2);
                    weightx = spdiags(1./weightx ,0, 2*sqrtn*(sqrtn-1),2*sqrtn*(sqrtn-1));
                    L = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
                end
            end
...
if ~precond
    ...
    if strcmpi(lname, 'Gradient1D') || strcmpi(lname, 'tv1D')
        np = n-1;
    elseif strcmpi(lname, 'Gradient2D') || strcmpi(lname, 'tv2D') || strcmpi(lname, 'diag')|| strcmpi(lname, 'atv2D')
        np = 2*sqrtn*(sqrtn-1);
    else
        np = n;
        %             np = P_solve(L, rand(n,1));
        %             np = length(np);
    end
    %catch
    %    error('Check the size of L')
    %end
end
