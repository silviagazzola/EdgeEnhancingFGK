...
%                   [ {'none'} | 'dwt' | 'svd' | 'tv1D' | 'tv2D' | 'atv2D' | 'diag']
...
defaultopt = struct('x0', 'none', 'MaxIter', 100 ,...
    ...
    'hybridvariant', 'I', 'tolX', 10^-10, 'updateWeights', 'on',...
    'GCVweight', 'adapt',...
...
updateWeights = IRget(options, 'updateWeights', [], 'fast');
...
m = length(b);
 ...
 % update weights in main loop
if strcmp(updateWeights,'off')
    precX_update = 0; % use initialised/first iterate weights for all iterates
else
    precX_update = 1; % update weights each iterate
end
...
elseif strcmp(SparsityTrans, 'dwt')
    ...
    TV = 0;
elseif strcmpi(SparsityTrans, 'tv1D') || strcmpi(SparsityTrans, 'tv2D') || strcmpi(SparsityTrans, 'diag')|| strcmpi(SparsityTrans, 'atv2D')
    TV = 1;
    Trans = speye(n);
end
...
if strcmp(x0, 'none')
    ...
    elseif TV
        ...
        if strcmpi(SparsityTrans, 'tv1D')
            dd = ones(n,1);
            D = spdiags([dd, -dd], 0:1, n-1, n); % D_1d
            if ischar(weightx)
                ...
                if max(abs(weightx)) == 0
                    ...
                    weightx = (weightx.^2 + tolX^2).^((q-2)/4);
        ...
        elseif strcmpi(SparsityTrans, 'tv2D')
            % variables necessary for standard form transformation
            p1 = vecN;
            nAN = norm(vecAN);
            pinvANtest = vecAN'/nAN^2;
            p2 = Atransp_times_vec(A, pinvANtest');
            ...
            weightx = IRget(options, 'weight0', [], 'fast'); %initial weights specified
            q = IRget(options, 'qnorm', [], 'fast');
            tolX      = IRget(options, 'tolX', [], 'fast');
            if ischar(weightx) % no weights given
                weightx = speye(2*sqrtn*(sqrtn-1));
                precX = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
            else
                try
                    isequal(length(weightx),2*sqrtn*(sqrtn-1));
                catch
                    error('Check the length of weight0 (an approximation of the solution, of coherent length, must be assigned)')
                end
                if max(abs(weightx)) == 0 % all weights zero
                    ...
                    % Calculate gradient
                    weightx = reshape(weightx,sqrtn,sqrtn);
                    Dhx = weightx(:,1:sqrtn-1)-weightx(:,2:sqrtn);
                    Dvx = weightx(1:sqrtn-1,:)-weightx(2:sqrtn,:);
                    % Calculate inverse TV weights
                    weightx =  (Dhx(:).^2 + Dvx(:).^2+tolX^2).^((2-q)/4);
        ...
        elseif strcmpi(SparsityTrans, 'atv2D')
            % variables necessary for standard form transformation
            p1 = vecN;
            nAN = norm(vecAN);
            pinvANtest = vecAN'/nAN^2;
            ...
            pinvDiag = sparse(1:n-1,1:n-1,1./spdiags(Diag(1:(n-1),:)),n,sqrtn*(sqrtn-1)*2);
            sizeP=n;
            % Compute weights for 2D Anisotropic TV
            weightx = IRget(options, 'weight0', [], 'fast');
            q = IRget(options, 'qnorm', [], 'fast');
            tolX      = IRget(options, 'tolX', [], 'fast');
            if ischar(weightx) % no weights given
                ...
                if max(abs(weightx)) == 0 % all weights zero
                    ...
                    Dhx = weightx(:,1:sqrtn-1)-weightx(:,2:sqrtn);
                    Dvx = weightx(1:sqrtn-1,:)-weightx(2:sqrtn,:);
                    % construct inverse edge-enhancing weights
                    weightx =  abs([Dhx(:);Dvx(:)]+tolX).^((2-q)/2);
                    weightx = spdiags(weightx ,0, 2*sqrtn*(sqrtn-1),2*sqrtn*(sqrtn-1)); % need to pass inverse weights
                    precX = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
                end
            end
        % PRECONDITIONER: Edge-enhancing diagonal weights (Gazzola, Kilmer, Nagy)
        elseif strcmpi(SparsityTrans,'diag')
            % variables necessary for standard form transformation
            p1 = vecN;
            nAN = norm(vecAN);
            pinvANtest = vecAN'/nAN^2;
            ...
            if ischar(weightx) % no weights given
                weightx = speye(2*sqrtn*(sqrtn-1));
                prevweight = ones(2*sqrtn*(sqrtn-1),1);
                precX = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
            else % weights given
                try
                    isequal(length(weightx),2*sqrtn*(sqrtn-1));
                ...
                % obtain weights from previous iterate, if available
                prevweight = IRget(options, 'weight1', [], 'fast');
                if ischar(prevweight)
                    prevweight = ones(2*sqrtn*(sqrtn-1),1);
                end
                q = IRget(options, 'qnorm', [], 'fast');
                if max(abs(weightx)) == 0 % all weights zero
                    ...
                    Dvx = weightx(1:sqrtn-1,:)-weightx(2:sqrtn,:);
                    g = [Dhx(:);Dvx(:)];
                    g = (1 - (abs(g)/max(abs(g))).^q).*prevweight ;% calculate weights
                    prevweight = g; % the noninverse weights are passed through to next iteration
                    g = 1+g; % alternative 'inverse' weights
                    g(g<tolX) = tolX; % ensure SVD is possible
                    weightx = spdiags(g ,0, 2*sqrtn*(sqrtn-1),2*sqrtn*(sqrtn-1));
                    precX = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
                end
            end
        end
...
% Establish whether preconditioning of the regularisation term of the projected problem is needed
if strcmpi(hybridvariant, 'I')
    Rfactor = 0;
elseif strcmpi(hybridvariant, 'R') || strcmpi(hybridvariant, 'Rnew')
    Rfactor = 1;
end
...
for k=1:MaxIter
    ...
    if Rfactor
        ...
        elseif strcmpi(hybridvariant, 'Rnew')
            if ~TV
                [ZUk, ZRk] = qr(Z(:,1:k)./precX, 0);
            else
                %Weights in precX correspond to the matrix W_2d^{-1}, used in
                %the un-projected regularisation term ||W_2d*D_2d*x||_2^2
                if k == 1 %Allocate memory
                    DZ = zeros(2*sqrtn*(sqrtn-1),max(K));
                end
                % Construct the matrix W_2d (values change each iterate)
                % note: if truepinv was used, then the value of weightx has
                % been changed. Instead, we extract weights directly from
                % precX
                weightk = precX(Z(:,k),'weights'); % retrieve the inverse weights
                % weightk = diag(weightk);
                if strcmpi(SparsityTrans,'diag') % used 'alternative' inverse weights
                    weightk = spdiags(weightk-1 ,0, 2*sqrtn*(sqrtn-1),2*sqrtn*(sqrtn-1));
                else 
                    weightk = spdiags(1./weightk ,0, 2*sqrtn*(sqrtn-1),2*sqrtn*(sqrtn-1));
                end
                %Calculate preliminary value D_2d*Z_k via column update
                Zcol = reshape(Z(:,k),sqrtn,sqrtn);
                DhZcol = Zcol(:,1:sqrtn-1)-Zcol(:,2:sqrtn);
                DvZcol = Zcol(1:sqrtn-1,:)-Zcol(2:sqrtn,:);
                % Update DZ matrix
                DZ(:,k) = [DhZcol(:);DvZcol(:)];
                % Calcualte W_2d*D_2d*Z
                WZ = weightk*DZ(:,1:k);                
                [ZUk, ZRk] = qr(WZ(:,1:k), 0);
            end
            ZRksq = ZRk(1:k,1:k);
    ...
    if precX_update
        ...
        elseif TV
            ...
            elseif strcmpi(SparsityTrans, 'tv2D')
                % Compute weights for 2D (isotropic) Total Variation
                weightx = x;
                if max(abs(weightx)) == 0 % all weights zero
                    weightx = speye(2*sqrtn*(sqrtn-1));
                    precX = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
                    ...
                    Dhx = weightx(:,1:sqrtn-1)-weightx(:,2:sqrtn);
                    Dvx = weightx(1:sqrtn-1,:)-weightx(2:sqrtn,:);
                    % calculate TV weights
                    weightx =  (Dhx(:).^2 + Dvx(:).^2+tolX^2).^((2-q)/4);
                    % invese weight matrix
                    weightx = spdiags([weightx;weightx] ,0, 2*sqrtn*(sqrtn-1),2*sqrtn*(sqrtn-1));
                    precX = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
                end
            % PRECONDITIONING: Anisotropic total variation
            elseif strcmpi(SparsityTrans, 'atv2D')
                % Compute weights for 2D anisoptropic total variation
                weightx = x;
                if max(abs(weightx)) == 0 % all weights zero
                    weightx = speye(2*sqrtn*(sqrtn-1));
                    precX = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
                    ...
                    % consturct weights
                    weightx =  abs([Dhx(:);Dvx(:)]+tolX).^((2-q)/2);
                    weightx = spdiags(weightx ,0, 2*sqrtn*(sqrtn-1),2*sqrtn*(sqrtn-1)); % pass inverse weights into precX
                    precX = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
                end
            elseif strcmpi(SparsityTrans,'diag')
                % Compute weights for edge-enhancing weighting
                weightx = x;
                if max(abs(weightx)) == 0 % all weights zero
                    weightx = speye(2*sqrtn*(sqrtn-1));
                    % no need to update prevweight
                    precX = @(xx,tflag) SNrightPrec_tv2D_Aw(xx, weightx, u_svd, v_svd, Q, pinvDiag, sizeP, p1, p2, tflag);
                else
                    % calculate gradient
                    weightx = reshape(weightx,sqrtn,sqrtn);
                    Dhx = weightx(:,1:sqrtn-1)-weightx(:,2:sqrtn);
                    Dvx = weightx(1:sqrtn-1,:)-weightx(2:sqrtn,:);
                    % calculate edge-enhancing weights
                    g = [Dhx(:);Dvx(:)];
                    g = (1 - (abs(g)/max(abs(g))).^q).*prevweight ;% calculate weights
                    prevweight = g; % update history of weights
